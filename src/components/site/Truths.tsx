...
const TRUTHS = [
  ...
  },
  {
    ...
    text: "Региональные вычеты (аренда, рождение ребёнка, спорт…), вычет для работающих матерей, пенсионные планы, доходы от аренды и инвестиций, IVA, Ceuta/Melilla, Страна Басков и Наварра. Autónomo без учёта art. 32.2 (для одного клиента). Всё это может сдвинуть итог на сотни евро.",
  },
];
      ...
      {TRUTHS.map((t, i) => (
        ...
        >
          <div
            ...
            style={{ background: "radial-gradient(circle, #ffb224, transparent 70%)" }}
          />
          <div className="eyebrow">{t.kicker}</div>
          <h3 className="mt-3 text-lg font-semibold leading-snug text-fg">{t.title}</h3>
          <p className="mt-3 text-[14px] leading-relaxed text-fg-2">{t.text}</p>
        </motion.article>
      ))}
    </div>
  );
